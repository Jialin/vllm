...
def test_find_longest_matched_ngram_and_propose_tokens_one_shot():
    status = NgramProposerStatus(min_ngram=2,
                                 max_ngram=2,
                                 max_model_len=1024,
                                 k=2)
    assert status.find_longest_matched_ngram_and_propose_tokens(
    ...
    np.testing.assert_array_equal(
        status.find_longest_matched_ngram_and_propose_tokens(tokens),
        np.array([4, 1, 2]))
    status = NgramProposerStatus(min_ngram=2,
                                 max_ngram=2,
                                 max_model_len=1024,
    ...
    np.testing.assert_array_equal(
        status.find_longest_matched_ngram_and_propose_tokens(tokens),
        np.array([4, 1]))
    status = NgramProposerStatus(min_ngram=1,
                                 max_ngram=1,
                                 max_model_len=1024,
                                 k=3)
    np.testing.assert_array_equal(
...
def test_find_longest_matched_ngram_and_propose_tokens_multi_round():
    status = NgramProposerStatus(min_ngram=4,
                                 max_ngram=4,
                                 max_model_len=1024,
                                 k=3)
    assert status.find_longest_matched_ngram_and_propose_tokens(
    ...
    assert status.find_longest_matched_ngram_and_propose_tokens(
        np.array([1, 2, 3, 4, 5, 1])) is None
    assert status.find_longest_matched_ngram_and_propose_tokens(
        np.array([1, 2, 3, 4, 5, 1, 2])) is None
    assert status.find_longest_matched_ngram_and_propose_tokens(
        np.array([1, 2, 3, 4, 5, 1, 2, 3])) is None
    np.testing.assert_array_equal(
